/*
  ...
  UPSHOPEE — /ofertas5
  ═══════════════════════════════════════════════════════════════
  Port da landing entregue como projeto Next.js (App Router) para
  a arquitetura real deste repo: Vite + TanStack Router.

  O que mudou em relação ao pacote de origem:
   · "use client" e o comentário de eslint do Next foram removidos;
   · o globals.css do pacote era global de verdade (redefinia :root,
     *, html, body, h1–h4, a, button). Aqui ele vive dentro de CSS,
     inteiramente escopado em `.o5x`, o wrapper da página. Nenhuma
     regra escapa — o dashboard e as outras landings ficam intactos;
   · os @keyframes viraram `o5x-*` para não colidirem com o app;
   · `--pricing-visible` é gravado no wrapper, não em
     document.documentElement, para não deixar resíduo global;
   · o metadata do layout.tsx do Next não foi portado — o index.html
     serve todas as rotas.

  Checkouts, preços, garantia e avisos legais são cópia literal do
  pacote (e batem com src/routes/ofertas4.tsx). Nenhum tracking novo
  e nenhum evento de compra no carregamento.
  ═══════════════════════════════════════════════════════════════
*/
  ...
  const paths: Record<IconName, ReactNode> = {
    ...
    pix: (
      <>
        ...
        <path d="m7 8-4 4 4 4M17 8l4 4-4 4M12 13l4 4-4 4-4-4 4-4Z" />
      </>
    ),
  };
  return (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Logo />
        <p className="eyebrow">FINALIZAR ACESSO</p>
        <h2 id="checkout-title">Como você prefere pagar?</h2>
        <p className="modal-plan">
          {isLifetime ? "Acesso Vitalício — R$ 259" : "Plano Mensal — R$ 145/mês"}
        </p>
        <div className="pay-options">
          <button onClick={() => window.location.assign(CHECKOUTS[plan].pix)}>
            <span className="pay-icon pix">
              <Icon name="pix" />
            </span>
            <span>
              <strong>Pagar com Pix</strong>
              <small>Liberação após a confirmação do pagamento</small>
            </span>
            <Icon name="arrow" />
          </button>
          <button onClick={() => window.location.assign(CHECKOUTS[plan].cartao)}>
            ...
            <span>
              ...
              <small>
                {isLifetime ? "Até 12x de R$ 27,61" : "Parcelamento disponível no checkout"}
              </small>
            </span>
            <Icon name="arrow" />
          </button>
        ...
        <p className="secure-line">
          <Icon name="shield" size={16} /> Pagamento seguro · 7 dias de garantia
        </p>
      </section>
    </div>
  );
...
.o5x .modal-backdrop{ position: fixed; z-index: 300; inset: 0; display: grid; place-items: center; padding: 20px; background: rgba(12,13,15,.62); backdrop-filter: blur(8px); animation: fadeIn .18s ease; }.o5x .checkout-modal{ position: relative; width: min(480px,100%); padding: 34px; border-radius: 22px; background: #fff; box-shadow: 0 35px 100px rgba(0,0,0,.28); animation: modalIn .25s ease; }.o5x .checkout-modal .brand-lockup{ margin-bottom: 25px; }.o5x .checkout-modal h2{ margin-bottom: 6px; font-size: 1.8rem; }.o5x .modal-plan{ margin-bottom: 25px; color: var(--orange); font-size: .84rem; font-weight: 750; }.o5x .modal-close{ position: absolute; right: 18px; top: 18px; width: 38px; height: 38px; display: grid; place-items: center; border: 1px solid var(--line); border-radius: 10px; background: #fff; }.o5x .pay-options{ display: grid; gap: 11px; }.o5x .pay-options > button{ width: 100%; display: grid; grid-template-columns: 48px 1fr auto; align-items: center; gap: 13px; padding: 14px; border: 1px solid var(--line); border-radius: 13px; background: #fff; text-align: left; transition: border-color .18s ease, transform .18s ease; }.o5x .pay-options > button:hover{ border-color: #eaa18e; transform: translateY(-2px); }.o5x .pay-options strong, .o5x .pay-options small{ display: block; }.o5x .pay-options strong{ margin-bottom: 4px; font-size: .86rem; }.o5x .pay-options small{ color: #888b91; font-size: .68rem; }.o5x .pay-options > button > svg{ color: #a4a6aa; }.o5x .pay-icon{ width: 46px; height: 46px; display: grid; place-items: center; border-radius: 11px; color: var(--orange); background: var(--orange-soft); }.o5x .pay-icon.pix{ color: #168860; background: #e9f7f1; }.o5x .secure-line{ display: flex; align-items: center; justify-content: center; gap: 7px; margin: 20px 0 0; color: #8b8e94; font-size: .68rem; }.o5x .secure-line svg{ color: var(--orange); }
.o5x .mobile-sticky{ display: none; }
@keyframes o5x-fadeIn { from { opacity: 0; } }
@keyframes o5x-modalIn { from { opacity: 0; transform: translateY(10px) scale(.98); } }
...
@media (max-width: 560px) {
  ...
  .o5x .checkout-modal{ padding: 29px 20px 24px; }.o5x .checkout-modal h2{ font-size: 1.55rem; }.o5x .pay-options > button{ grid-template-columns: 43px 1fr auto; padding: 12px; }.o5x .pay-icon{ width: 42px; height: 42px; }}
...
function Ofertas5Page() {
  ...
  useEffect(() => {
    document.body.style.overflow = menuOpen || checkoutPlan ? "hidden" : "";
    return () => {
      document.body.style.overflow = "";
    };
  }, [menuOpen, checkoutPlan]);
  ...
  return (
    <div className="o5x" ref={rootRef}>
        ...
        <div className="container nav-wrap">
          ...
          <div className="nav-actions">
            <button
              ...
              onClick={() => setCheckoutPlan("vitalicio")}
            >
      ...
      <main>
        ...
        <section className="section pricing-section" id="planos" ref={pricingRef}>
          ...
          <div className="container payment-note">
            <Icon name="shield" />
            <span>
              <strong>Pagamento processado em ambiente seguro.</strong> Escolha Pix ou cartão antes
        ...
        <section className="final-cta">
          <div className="container">
            ...
            <button className="button button-primary" onClick={() => setCheckoutPlan("vitalicio")}>
      ...
      <div className={`mobile-sticky ${stickyVisible ? "visible" : ""}`}>
        ...
        <button onClick={() => setCheckoutPlan("vitalicio")}>
