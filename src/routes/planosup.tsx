...
export const Route = createFileRoute("/planosup")({ component: PlanosPage });
  ...
  return (
    <nav style={{
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 20px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <Link to="/ofertas" style={{ display: "flex", alignItems: "center", gap: 9, textDecoration: "none", flexShrink: 0 }}>
          <img src="/brand/logo.png" alt="UpShopee" style={{ height: 34, width: 34, objectFit: "contain" }} />
...
function PricingSection() {
  const [paymentModal, setPaymentModal] = useState<{ open: boolean; plan: 'mensal' | 'vitalicio' | null }>({ open: false, plan: null });

  useEffect(() => {
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') setPaymentModal({ open: false, plan: null }); };
    ...
    <section id="planos" style={{ padding: '100px 20px', background: '#080808', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 880, margin: '0 auto' }}>
        ...
        <div className="text-center mb-8">
          <p className="text-white font-black text-2xl md:text-3xl reveal">Esse é o valor da sua liberdade financeira hoje</p>
          <p className="text-[#EE4D2D] font-bold text-lg mt-2 reveal">💸 Menos que um delivery por mês</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,340px),1fr))', gap: 20, alignItems: 'start' }}>
          ...
          <div className="reveal pricing-card" style={{ background: '#111111', border: '1px solid #1F1F1F', borderRadius: 20, padding: '36px 32px' }}>
            <div style={{ display: 'inline-block', background: 'rgba(238,77,45,0.12)', border: '1px solid rgba(238,77,45,0.25)', borderRadius: 999, padding: '4px 14px', marginBottom: 16, fontSize: 10, fontWeight: 700, color: '#EE4D2D', textTransform: 'uppercase' as const, letterSpacing: '0.06em', fontFamily: "'Barlow', sans-serif" }}>PARA COMEÇAR</div>
            <h3 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 4px', color: '#FFFFFF', fontFamily: "'Barlow', sans-serif" }}>Mensal</h3>
            <p style={{ fontSize: 13, color: '#888888', margin: '0 0 24px', fontFamily: "'Barlow', sans-serif" }}>Para quem quer começar com baixo investimento.</p>
            <div style={{ marginBottom: 28 }}>
              <div style={{ fontSize: 13, color: '#888888', textDecoration: 'line-through', marginBottom: 2, fontFamily: "'Barlow', sans-serif" }}>De R$247</div>
              <span style={{ fontSize: 44, fontWeight: 900, letterSpacing: '-2px', color: '#EE4D2D', fontFamily: "'Barlow', sans-serif" }}>R$167</span>
            ...
            <div className="flex items-center justify-center gap-4 mt-3 text-xs text-[#888]"><span>🔒 Pagamento Seguro</span><span>🛡️ 7 dias de garantia</span></div>
          </div>
          ...
          <div className="reveal pricing-card" style={{ background: 'linear-gradient(145deg, rgba(238,77,45,0.08) 0%, #111111 50%)', border: '1.5px solid #EE4D2D', borderRadius: 20, padding: '36px 32px', position: 'relative', boxShadow: '0 0 60px rgba(238,77,45,0.15)', transitionDelay: '0.1s' }}>
            <div style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg,#EE4D2D,#F97316)', borderRadius: 999, padding: '5px 20px', fontSize: 12, fontWeight: 700, color: '#fff', whiteSpace: 'nowrap' as const, letterSpacing: '0.04em', fontFamily: "'Barlow', sans-serif" }}>MAIS POPULAR</div>
            <h3 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 4px', color: '#FFFFFF', fontFamily: "'Barlow', sans-serif" }}>Acesso Vitalício</h3>
            <p style={{ fontSize: 11, fontWeight: 600, color: '#F59E0B', margin: '0 0 20px', fontFamily: "'Barlow', sans-serif" }}>Oferta de Lançamento — Encerrando em Breve</p>
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 13, color: '#888888', textDecoration: 'line-through', marginBottom: 2, fontFamily: "'Barlow', sans-serif" }}>De R$497</div>
              <div>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#EE4D2D', marginRight: 4, fontFamily: "'Barlow', sans-serif" }}>Por</span>
                <span style={{ fontSize: 46, fontWeight: 900, letterSpacing: '-2px', color: '#EE4D2D', fontFamily: "'Barlow', sans-serif" }}>R$267</span>
              </div>
              <div style={{ fontSize: 12, color: '#888888', marginTop: 4, fontFamily: "'Barlow', sans-serif" }}>ou 12x de R$27,61 no cartão</div>
              <div style={{ fontSize: 12, color: '#888888', marginTop: 2, fontFamily: "'Barlow', sans-serif" }}>Pagamento único — acesso para sempre</div>
            </div>
            ...
            <button onClick={() => setPaymentModal({ open: true, plan: 'vitalicio' })}
              className="lp-shimmer"
            ...
            <p style={{ fontSize: 12, color: '#888888', textAlign: 'center', marginTop: 10, fontFamily: "'Barlow', sans-serif" }}>💳 Parcelamento disponível — consulte no checkout</p>
            <div className="flex items-center justify-center gap-4 mt-3 text-xs text-[#888]"><span>🔒 Pagamento Seguro</span><span>🛡️ 7 dias de garantia</span></div>
          </div>
        </div>

        <div className="flex items-center justify-center gap-2 mt-8 text-white/50 text-sm reveal"><span>🛡️</span><span>7 dias de garantia total — devolvemos 100% se não gostar</span></div>
      </div>

      {paymentModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setPaymentModal({ open: false, plan: null })}>
          <div className="bg-[#0F0F1A] rounded-3xl p-8 max-w-sm w-full border border-white/10" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-center mb-4"><div className="w-16 h-16 rounded-2xl bg-[#EE4D2D] flex items-center justify-center text-3xl">🚀</div></div>
            <p className="text-[#EE4D2D] font-black text-xs uppercase tracking-widest text-center mb-2">{paymentModal.plan === 'mensal' ? 'PLANO MENSAL' : 'PLANO VITALÍCIO'}</p>
            <h2 className="text-white font-black text-4xl text-center mb-1">{paymentModal.plan === 'mensal' ? 'R$167' : 'R$267'}</h2>
            <p className="text-white/50 text-sm text-center mb-6">{paymentModal.plan === 'mensal' ? 'por mês · cancele quando quiser' : 'pagamento único — acesso vitalício'}</p>
            <hr className="border-white/10 mb-6" />
            <p className="text-white/60 text-sm text-center mb-4">Como prefere pagar?</p>
            <div className="flex flex-col gap-3 mb-4">
              <a href={LINKS[paymentModal.plan!].pix} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 bg-[#0D2B1F] border-2 border-[#32BCAD] rounded-2xl py-5 px-5 w-full hover:bg-[#0D3524] transition-all">
                <svg viewBox="0 0 512 512" width="28" height="28" fill="#32BCAD" style={{ flexShrink: 0 }}><path d="M242.4 292.5C247.8 287.1 255.1 284.1 262.5 284.1C269.9 284.1 277.2 287.1 282.6 292.5L364.1 374C377 386.9 393.9 394.1 412.1 394.1H427.4L344.6 476.8C319.9 501.5 279.6 501.5 254.9 476.8L172.8 394.1H188.4C206.6 394.1 223.5 386.9 236.4 374L242.4 292.5zM282.6 219.5C277.2 224.9 269.9 227.9 262.5 227.9C255.1 227.9 247.8 224.9 242.4 219.5L160.1 138C147.2 125.1 130.3 117.9 112.1 117.9H96.8L179.6 35.2C204.3 10.5 244.6 10.5 269.3 35.2L352.1 117.9H336.5C318.3 117.9 301.4 125.1 288.5 138L282.6 219.5z"/></svg>
                <div><p className="text-white font-bold text-base">PIX</p><p className="text-[#32BCAD] text-xs mt-0.5">Aprovação instantânea</p></div>
              </a>
              <a href={LINKS[paymentModal.plan!].cartao} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 bg-[#1A1030] border-2 border-purple-700 rounded-2xl py-5 px-5 w-full hover:bg-[#1E1438] transition-all">
                <span className="text-2xl" style={{ flexShrink: 0 }}>💳</span>
                <div><p className="text-white font-bold text-base">Cartão de Crédito</p><p className="text-purple-400 text-xs mt-0.5">Até 12x · Visa, Master, Elo, Amex</p></div>
              </a>
            </div>
            <div className="flex items-center justify-center gap-2 text-white/40 text-xs mb-4"><span>🔒 Pagamento 100% seguro · 🛡️ 7 dias de garantia</span></div>
            <button onClick={() => setPaymentModal({ open: false, plan: null })} className="w-full text-white/40 text-sm hover:text-white transition-colors py-2">Fechar</button>
          </div>
        </div>
      )}
    </section>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 32, justifyContent: "space-between", alignItems: "flex-start", marginBottom: 40 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 10 }}>
              <img src="/brand/logo.png" alt="UpShopee" style={{ height: 30, width: 30, objectFit: "contain" }} />
