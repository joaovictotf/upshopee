...
CREATE OR REPLACE FUNCTION public.admin_create_boost_campaign(
  _user_id uuid,
  _pack_id text,
  _starts_at timestamp with time zone DEFAULT now(),
...
BEGIN
  ...
  CASE _pack_id
    ...
    WHEN 'maximo' THEN
      ...
      _fixed_comm := 200;
    ELSE
      RAISE EXCEPTION 'Pack inválido';
  END CASE;

  -- Require at least one product from this user's "Meus Produtos"
  SELECT EXISTS (
    SELECT 1 FROM public.user_products WHERE user_id = _user_id
  ) INTO _has_any_product;
  IF NOT _has_any_product THEN
  ...
  FOR _i IN 1.._total_sales LOOP
    ...
    INSERT INTO public.boost_simulated_events(
      ...
      _cid, _user_id, _prod.id, _prod.name, _prod.image, _ts, _comm
    );
  END LOOP;
